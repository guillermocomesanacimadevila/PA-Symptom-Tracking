...
def treated_cluster_support(events_df, id_col="id", treat_col="A_star"):
    treated = events_df[events_df[treat_col] == 1]
    if treated.empty:
        return 0, 0.0
    counts = treated[id_col].value_counts().to_numpy()
    p = counts / counts.sum()
    G = len(counts)
    G_eff = 1.0 / np.sum(p ** 2)
    return G, G_eff

# 1 = intramuscular injection; 2 = subcutaneous injection
# treatment_type = 1 / 2
def find_treatment_episodes(df, id_col, day_col, treat_col, treat_type_col, inj_codes=(1, 2)):
    eps = []
    df = df.copy()
    df[day_col] = pd.to_numeric(df[day_col], errors="coerce")
    df[treat_col] = pd.to_numeric(df[treat_col], errors="coerce").fillna(0).astype(int)
    df[treat_type_col] = pd.to_numeric(df[treat_type_col], errors="coerce")

    for pid in df[id_col].dropna().unique():
        g = df[df[id_col] == pid].sort_values(day_col)
        tdf = g[(g[treat_col] == 1) & (g[treat_type_col].isin(inj_codes))]
        days = [int(d) for d in tdf[day_col].tolist() if pd.notna(d)]
        if not days:
            continue
        start = prev = days[0]
        for d in days[1:]:
...
def build_treated_events(df, L, H, id_col, day_col, treat_col, treat_type_col, outcome_col, inj_codes=(1, 2)):
    rows = []
    eps = find_treatment_episodes(df, id_col, day_col, treat_col, treat_type_col, inj_codes=inj_codes)
    df = df.copy()
    df[day_col] = pd.to_numeric(df[day_col], errors="coerce")
    df[treat_col] = pd.to_numeric(df[treat_col], errors="coerce").fillna(0).astype(int)
    df[treat_type_col] = pd.to_numeric(df[treat_type_col], errors="coerce")
    for pid, start, end in eps:
        ...
        if ((win_course[treat_col] != 1) | (~win_course[treat_type_col].isin(inj_codes))).any():
            continue

        reinj_in_post = ((win_post[treat_col] == 1) & (win_post[treat_type_col].isin(inj_codes))).any()
        if reinj_in_post:
            continue
        ...
        baseline = float(win_pre[outcome_col].iloc[-1])
...
def build_control_events(df, L, H, id_col, day_col, treat_col, outcome_col, max_per_id=None):
    ...
    for pid in df[id_col].dropna().unique():
        ...
        for c in days:
            ...
            win_pre = win[win[day_col].isin(pre)]
            baseline = float(win_pre[outcome_col].iloc[-1])
            future = float(win_post[outcome_col].mean())
            y_star = future - baseline
            row = row_c.to_dict()
            row["A_star"] = 0
            row["Y_star"] = float(y_star)
