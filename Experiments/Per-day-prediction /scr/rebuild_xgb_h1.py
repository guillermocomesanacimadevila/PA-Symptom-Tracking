...
def _load_best_params_from_cv(cv_path: str) -> Dict[str, Any]:
    df = pd.read_csv(cv_path)
    if "rank_test_r2" in df.columns:
        best_idx = df["rank_test_r2"].idxmin()
    else:
        best_idx = df["mean_test_r2"].idxmax()
    row = df.loc[best_idx]
    best_params = {}
    for col in df.columns:
        if col.startswith("param_") and not pd.isna(row[col]):
            key = col[len("param_"):]
            best_params[key] = row[col]
    return best_params
...
def main():
    ...
    for h in range(1, H_MAX + 1):
        ...
        if h == 1:
            cv_path = os.path.join(out_dir_h, f"{name}_cv_results.csv")
            if not os.path.exists(cv_path):
                raise FileNotFoundError(cv_path)
            best_params = _load_best_params_from_cv(cv_path)
            best_cv_csv = cv_path
    ...
    summary_path = os.path.join(OUT_DIR, "summary_xgb_all_horizons.csv")
