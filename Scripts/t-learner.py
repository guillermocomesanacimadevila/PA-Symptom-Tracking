...
def main():
    ...
    for name in model_names:
        ...
        y1_train = predict_pipeline_compat(model_treated, X_train)
        y0_train = predict_pipeline_compat(model_control, X_train)
        A_tr = a_train.astype(float)
        Y_tr = y_train.astype(float)
        tau_dr_train = (
            (A_tr / ps_train) * (Y_tr - y1_train)
            - ((1.0 - A_tr) / (1.0 - ps_train)) * (Y_tr - y0_train)
            + y1_train - y0_train
        )
